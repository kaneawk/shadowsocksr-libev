...
static int g_useragent_post_index = -1;

typedef struct http_post_local_data {
    int has_sent_header;
    int has_recv_header;
    char *encode_buffer;
}http_post_local_data;

void http_post_local_data_init(http_post_local_data* local) {
    local->has_sent_header = 0;
    local->has_recv_header = 0;
    local->encode_buffer = NULL;

    if (g_useragent_post_index == -1) {
        g_useragent_post_index = xorshift128plus() % (sizeof(g_useragent_post) / sizeof(*g_useragent_post));
    }
}

obfs * http_post_new_obfs() {
    obfs * self = new_obfs();
    self->l_data = malloc(sizeof(http_post_local_data));
    http_post_local_data_init((http_post_local_data*)self->l_data);
    return self;
}

void http_post_dispose(obfs *self) {
    http_post_local_data *local = (http_post_local_data*)self->l_data;
...
void http_post_encode_head(http_post_local_data *local, char *data, int datalength) {
    if (local->encode_buffer == NULL) {
    ...
    for (; pos < datalength; ++pos) {
        ...
        local->encode_buffer[pos * 3 + 1] = http_post_hex(((unsigned char)data[pos] >> 4));
        local->encode_buffer[pos * 3 + 2] = http_post_hex(data[pos] & 0xF);
    }
...
int http_post_client_encode(obfs *self, char **pencryptdata, int datalength, size_t* capacity) {
    ...
    http_post_encode_head(local, encryptdata, head_size);
    ...
    if (body_buffer) {
        ...
        sprintf(out_buffer,
            ...
            local->encode_buffer,
            hostport,
            g_useragent_post[g_useragent_post_index],
            result
            );
    }
...
int http_post_client_decode(obfs *self, char **pencryptdata, int datalength, size_t* capacity, int *needsendback) {
    char *encryptdata = *pencryptdata;
    http_post_local_data *local = (http_post_local_data*)self->l_data;
    *needsendback = 0;
